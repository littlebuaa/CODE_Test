...
class JigITT:
	...
	def getNo2InjectionTime(self, current_ppm, target_ppm):
		...
		# Interpolation of current time
		# 查表找到与当前浓度最近的两个值(Ta Pa)和(Tb Pb)，用这两对值进行线性估计，Current浓度所需时间 = Ta + (C-Pa)  *    (Tb-Ta)/(Pa-Pb)
		idx_a = 0
		val_a = no2[0].ppm
		idx_b = None
		val_b = None
		...
		# Interpolation of target time
		# TODO: hey ! this is a nice copy/paste of above...
		# # 查表找到与目标最近的两个值(Ta Pa)和(Tb Pb)，用这两对值进行线性估计，Target浓度所需时间 = Ta + (T-Pa)  *    (Tb-Ta)/(Pa-Pb)
		idx_a = 0
		val_a = no2[0].ppm
		idx_b = None
		val_b = None
